...
class TestOutputFormats:
    ...
    def test_create_regions_services_csv(self):
        ...
        create_regions_services_csv(self.config, self.test_regions, self.test_region_services, quiet=True)
        
        csv_file = Path(self.temp_dir) / "test_regions.csv"
    ...
    def test_create_services_regions_matrix_csv(self):
        ...
        create_services_regions_matrix_csv(self.config, self.test_regions, self.test_region_services, quiet=True)
        
        csv_file = Path(self.temp_dir) / "test_matrix.csv"
        assert csv_file.exists()
    ...
    def test_create_json_output(self):
        ...
        result = create_json_output(self.config, self.test_regions, self.test_region_services, self.test_metadata, quiet=True)
        
        assert result is True
        
        json_file = Path(self.temp_dir) / "test_regions.json"
        ...
        assert data["generator"]["version"] == "1.2.0"
        ...
        assert summary["least_available_service"] in ["lambda"]  # Could be lambda (1 region)
    ...
    def test_create_excel_output_success(self):
        ...
        try:
            result = create_excel_output(self.config, self.test_regions, self.test_region_services, self.test_metadata, quiet=True)
            
            # If pandas/openpyxl are available, should succeed
            if result:
                excel_file = Path(self.temp_dir) / "test_regions.xlsx"
    ...
    def test_create_excel_output_missing_dependencies(self):
        ...
        with patch('builtins.__import__', side_effect=lambda name, *args: 
                  ImportError("pandas not available") if name == 'pandas' else __import__(name, *args)):
            result = create_excel_output(self.config, self.test_regions, self.test_region_services, quiet=True)
    ...
    def test_empty_data_handling(self):
        ...
        result = create_json_output(self.config, empty_regions, empty_services, quiet=True)
        assert result is True
    ...
    def test_file_creation_errors(self):
        ...
        result = create_json_output(bad_config, self.test_regions, self.test_region_services, quiet=True)
        assert result is True
    ...
    def test_large_data_handling(self):
        ...
        large_services = {f"region-{i:03d}": [f"service-{j}" for j in range(i % 10)] for i in range(100)}
        
        # Test JSON output with large data
        result = create_json_output(self.config, large_regions, large_services, quiet=True)
        assert result is True
    ...
    def test_special_characters_handling(self):
        ...
        result = create_json_output(self.config, special_regions, special_services, quiet=True)
        assert result is True
