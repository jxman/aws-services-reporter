...
def create_excel_output(config: Config, regions: Dict[str, str], region_services: Dict[str, List[str]], 
    ...
    """
    output_path = Path(config.output_dir) / "excel" / f"{Path(config.regions_filename).stem}.xlsx"
    ...
    try:
        ...
        for service in all_services:
            ...
            for region_code in sorted_regions:
                available = "✓" if service in region_services.get(region_code, []) else "✗"
        ...
        most_available = max(service_coverage.keys(), key=lambda x: service_coverage[x]) if service_coverage else "N/A"
        least_available = min(service_coverage.keys(), key=lambda x: service_coverage[x]) if service_coverage else "N/A"
        ...
        stats_data = [
            ...
            ["Most Available Service", f"{most_available} ({service_coverage.get(most_available, 0)} regions)"],
            ["Least Available Service", f"{least_available} ({service_coverage.get(least_available, 0)} regions)"],
        ]
        ...
        if not quiet:
            print(f"    ✓ Created Excel output ({file_size:,} bytes, {len(wb.worksheets)} sheets)")
